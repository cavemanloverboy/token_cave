...
fn test_payment_payout() {
    ...
    let operator: User = get_funded_user(&dev_key, &mint_key, &solana_client)
        .expect("failed to get funded user");

    // Get tss placeholder (NOTE: this account is currently unchecked)
    let tss_placeholder: User = get_funded_user(&dev_key, &mint_key, &solana_client)
            .expect("failed to get funded user");
    ...
    let cave_info_account: CaveTunnelInfo = program
        .account(cave_tunnel_info)
        .unwrap();
    assert_eq!(
        cave_info_account.service_time,
        DEMO_REQUEST_SERVICE_TIME,
    );
